...
//  PassengerApp
//
//  Created by Tarwinder Singh on 21/04/18.
...
open class Bottomsheet {
    open class Controller: UIViewController {
        typealias viewDismissHandler = (_ isViewDismiss:Bool) -> Void
        ...
        var viewDismissHandler:viewDismissHandler!
        ...
        fileprivate var scrollView: UIScrollView?
         var isScrollEnabledInSheet: Bool = true
        ...
        open func addNavigationbar(_ configurationHandler: ((UINavigationBar) -> Void)? = nil) {
            ...
            let heightConstraint = NSLayoutConstraint(item: navigationBar,
                                                      ...
                                                      attribute: .height,
                                                      multiplier: 1,
                                                      constant: navigationBarHeight) // navigationBarHeight + statusBarHeight
...
//            navigationBar.sizeThatFits(CGSize(width: Application.screenSize.width, height: 64))
        ...
        func setViewDismissHandler(viewDismissHandler: @escaping viewDismissHandler){
            self.viewDismissHandler = viewDismissHandler
        }
        
        func dismissView(viewDismissHandler: @escaping viewDismissHandler){
            self.viewDismissHandler = viewDismissHandler
            state = .hide
        }
...
private extension BottomsheetController {
    ...
    func transform(_ state: State) {
        ...
        case .hide:
            ...
            let completion: ((Bool) -> Void) = { [weak self] in
                ...
                self?.dismiss(animated: true, completion: {
                    if(self?.viewDismissHandler != nil){
                        self?.viewDismissHandler(true)
                    }
                })
            }
        ...
        case .showAll:
            ...
            let animations: (() -> Void) = {
                if(self.bar != nil && self.bar!.isKind(of: CustomNavBar.self)){
                    let navBar = self.bar as! CustomNavBar
                    if(navBar.subviews.count > 0){
                        for i in 0..<navBar.subviews.count {
                            navBar.subviews[i].frame.origin.y = navBar.subviews[i].frame.origin.y + self.statusBarHeight
                        }
                    }
                    navBar.changeHeight(customHeight: self.navigationBarHeight + self.statusBarHeight)
                }
...
extension BottomsheetController: UIGestureRecognizerDelegate {
    public func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if(gestureRecognizer.view != nil && (gestureRecognizer.view!.isKind(of: UINavigationBar.self) || gestureRecognizer.view!.isKind(of: UIToolbar.self))){
            return true
        }
//        if(gestureRecognizer.view != nil && gestureRecognizer.view!.isKind(of: UIScrollView.self)){
//            let scrollView = gestureRecognizer.view! as! UIScrollView
//            if(scrollView.isAtTop == true){
//                return true
//            }
//        }
