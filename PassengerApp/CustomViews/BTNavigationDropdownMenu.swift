...
open class BTNavigationDropdownMenu: UIView {
    ...
    open var didSelectItemAtIndexHandler: ((_ indexID: String) -> ())?
    open var menuStateHandler: ((_ isMenuOpen: Bool) -> ())?
    ...
    public init(navigationController: UINavigationController? = nil, containerView: UIView = UIApplication.shared.keyWindow!, title: String, items: [NSDictionary]) {
        // Key window
        ...
        self.tableView.selectRowAtIndexPathHandler = { [weak self] (indexID: String) -> () in
            guard let selfie = self else {
                return
            }
            selfie.didSelectItemAtIndexHandler!(indexID)
    ...
    open func updateItems(_ items: [NSDictionary]) {
        if !items.isEmpty {
...
class BTConfiguration {
    ...
    func defaultValue() {
        // Path for image
        _ = Bundle(for: BTConfiguration.self)
        ...
        self.checkMarkImage = UIImage(named: "ic_arrow_right")
        self.shouldKeepSelectedCellColor = false
        self.animationDuration = 0.5
        //        self.arrowImage = UIImage(contentsOfFile: arrowImagePath!)
        self.arrowImage = UIImage(named: "ic_arrow_right")
...
class BTTableView: UITableView, UITableViewDelegate, UITableViewDataSource {
    ...
    var selectRowAtIndexPathHandler: ((_ indexID: String) -> ())?
    
    // Private properties
    fileprivate var items: [NSDictionary]!
    fileprivate var selectedIndexPath = ""
    
    required init?(coder aDecoder: NSCoder) {
        fatalError("init(coder:) has not been implemented")
    }
    
    init(frame: CGRect, items: [NSDictionary], title: String, configuration: BTConfiguration) {
        super.init(frame: frame, style: UITableViewStyle.plain)
        
        self.items = items
        //        self.selectedIndexPath = item.get("ID")
        
        //            (items as! [String]).index(of: title)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = self.items[indexPath.item].get("Title")
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        selectedIndexPath = selectedItem.get("ID")
        
        self.selectRowAtIndexPathHandler?(selectedIndexPath)
        self.reloadData()
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if self.configuration.shouldKeepSelectedCellColor == true {
            ...
            cell.contentView.backgroundColor = self.configuration.cellBackgroundColor
            //            ((indexPath as NSIndexPath).row == selectedIndexPath) ? self.configuration.cellSelectionColor :
        }
    }
}
