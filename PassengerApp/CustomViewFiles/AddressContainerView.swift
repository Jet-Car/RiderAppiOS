...
//        GeneralFunctions.setImgTintColor(imgView: pickUpLocImgView, color: UIColor(hex: 0x1c730b))
    ...
    func setLocationIndicator(){
        if(self.mainScreenUv != nil && self.mainScreenUv.isDriverAssigned == true && self.mainScreenUv.userProfileJson.get("IS_DEST_ANYTIME_CHANGE").uppercased() == "YES" && self.mainScreenUv.eTripType.uppercased() == Utils.cabGeneralType_Ride.uppercased() && self.mainScreenUv.eFlatTrip == false){
            ...
            if(self.mainScreenUv.destAddress != ""){
                //If rental than hide edit destination
               // if self.mainScreenUv.isCurrentTripRental{
        ...
        if(isPickUpMode == false){
            ...
            if(self.mainScreenUv != nil && (self.mainScreenUv.isDriverAssigned == false || (self.mainScreenUv.isDriverAssigned == true && (self.mainScreenUv.destLocation == nil || self.mainScreenUv.destAddress == "") ))){
               self.continueDestSelection()
                ...
                if(self.mainScreenUv.isDriverAssigned == true && self.mainScreenUv.userProfileJson.get("IS_DEST_ANYTIME_CHANGE").uppercased() == "YES" && self.mainScreenUv.eTripType.uppercased() == Utils.cabGeneralType_Ride.uppercased() && self.mainScreenUv.eFlatTrip == false){
                    //If rental than not able to edit
                  //  if(self.mainScreenUv.isCurrentTripRental){
                  //      return
                  //  }
                    self.continueDestSelection()
