...
//        }
        ...
        if(self.uv .isKind(of: MainScreenUV.self) && (self.uv as! MainScreenUV).isFromUFXhomeScreen == true){
    ...
    func myBtnTapped(sender: MyButton) {
        if(InternetConnection.isConnectedToNetwork() == false)
        {
            if #available(iOS 10.0, *) {
                if(GeneralFunctions.getSettingsSchema() != ""){
                    UIApplication.shared.open(URL(string: GeneralFunctions.getSettingsSchema())!, options: [:], completionHandler: nil)
                }else{
            ...
            if (CLLocationManager.locationServicesEnabled()  == false) {
                if(GeneralFunctions.getLocationSettingsSchema() != ""){
                    if let url = URL(string: GeneralFunctions.getLocationSettingsSchema()) {
